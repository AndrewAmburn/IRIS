...
def main():
    ...
    model_path = os.path.join(script_dir, '../IRIS_models/RL_dock_solv.pkl')  # Model located in IRIS_models relative to this script
    descriptors_path = os.path.join(directory, 'descriptors.txt')
    sdf_file_path = os.path.join(directory, f"{folder_name}_docking_out_sorted.sdf")  # Construct SDF file name based on folder name
    output_sdf_path = os.path.join(directory, f"{folder_name}_predicted_sorted.sdf")
